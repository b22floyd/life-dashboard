...
- `src/lib/cleaning.ts` — server-only `getCleaningTasks()`: fetches tasks with their nested completions, takes the most recent `completed_at` per task, and runs it through `computeCleaningStatus`.
