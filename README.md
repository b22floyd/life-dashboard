...
- `src/components/dashboard/EventsCard.tsx` (Server Component, fetches data + handles connect/disconnect/error states) + `EventsCardBody.tsx` (Client Component) — splits events into a **Today** section (time only) and an **Upcoming** section (date + time), grouped by the browser's local calendar day (see [Timezones](#timezones)), each independently capped at ~5 visible items with a scrollbar for the rest.
