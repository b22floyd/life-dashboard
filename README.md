...
- Check-in notes follow the same collapsed-by-default `▸`/`▾` toggle pattern as Weight Tracker's History and Weight Training's Session History, newest-first, capped at `max-h-48 overflow-y-auto`. Note timestamps are formatted client-side (mount-gated) since they're absolute instants, not date-only strings (see [Timezones](#timezones)).
